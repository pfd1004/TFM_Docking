...
OUTDIR = Path("resultados/modelos/modelo_final_rdkit")
...
# Quitar SDS si no tiene membrana no importa, porque este modelo usa RDKit solo.
# Pero quitamos columnas mem_ para que sea exactamente RDKit.
numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()

feature_cols = [
    c for c in numeric_cols
    if c not in TARGETS
    and not c.startswith("mem_")
    and c != "n_layerposition"
]

# FormalCharge suele ser constante, se puede dejar porque VarianceThreshold la quita.
...
}).to_csv(OUTDIR / "features_modelo_final.csv", index=False)
...
with open(OUTDIR / "descripcion_modelo_final.txt", "w", encoding="utf-8") as f:
    ...
    f.write("Hiperparámetros seleccionados:\n")
    f.write("n_estimators=300\n")
    f.write("max_depth=None\n")
    f.write("max_features=0.5\n")
    f.write("min_samples_leaf=1\n")

print("Modelo final entrenado con todos los ligandos disponibles.")
