...
Para integrar de forma exploratoria la información ligando-proteína y nanopartícula-proteína se construyó una tabla en formato largo con una fila por combinación ligando-nanopartícula-proteína. La afinidad ligando-proteína se denotó como \(E^{LP}_{i,p}\), donde \(i\) es el ligando y \(p\) la proteína. La afinidad nanopartícula-proteína se denotó como \(E^{NP}_{n,p}\), donde \(n\) es la nanopartícula.

Como valores más negativos indican interacciones más favorables, se transformaron las energías en puntuaciones normalizadas por proteína:

```{=latex}
\[
S^{LP}_{i,p} = -z_p\left(E^{LP}_{i,p}\right)
\]
```

```{=latex}
\[
S^{NP}_{n,p} = -z_p\left(E^{NP}_{n,p}\right)
\]
```

donde \(z_p\) representa el z-score calculado dentro de cada proteína. A continuación, se definió un score contextual medio:

$$
S^{contexto}_{i,n,p} = \frac{S^{LP}_{i,p} + S^{NP}_{n,p}}{2}
$$Este score no representa una energía física total. Su función es ordenar combinaciones de forma comparativa, integrando dos tendencias: ligandos con afinidad favorable frente a una proteína y nanopartículas con interacción favorable frente a esa misma proteína.
